...
def translate_table(table_data, target_language="ko"):
    translated_table = []
...
def process_pdf_elements(pdf_elements, target_language="ko"):
    for page_elements in pdf_elements:
        ...
        for img_dict in page_elements["images"]:
            ...
            if image_data:
                translated_image_data = ocr_and_translate_image(
                    image_data, target_language
                )
                if translated_image_data:
                    img_dict["image"] = (
                        translated_image_data  # 번역된 이미지 데이터 저장
                    )
    return pdf_elements
